...
/*TABLE TYPE OF PARCEL*/
 CREATE TABLE BO_LOCKER(
  COD_LOCKER      BIGINT PRIMARY KEY AUTO_INCREMENT   COMMENT 'COD OF THE PERSON'
  ,COD_PEOPLE     BIGINT                              COMMENT 'COD OF THE PERSON',
  ,NAME_LOCKER    VARCHAR(120) NOT NULL UNIQUE        COMMENT 'THE NAME OF THE USER'
  ,CLAVE          VARCHAR(2000) NOT NULL              COMMENT 'THE PASSWORD OF THE USER'
  ,IND_LOCK       ENUM('1','0')  NOT NULL             COMMENT 'IND OF THE LOCKER TO ACCESS THE SYSTEM, 0=INACTIVE 1=ACTIVE'
  ,DAT_UPD        DATETIME NOT NULL                   COMMENT 'DATE THIS ROW WAS MODIFIED'
  ,DAT_ADD        DATETIME NOT NULL                   COMMENT 'DATE THAT THIS ROW WERE ADDED'
  ,FOREIGN KEY `FK_PEOADD_PEOLOCKER`(`COD_PEOPLE`) REFERENCES `PA_PEOPLE`(`COD_PEOPLE`) ON DELETE CASCADE
 )ENGINE=INNODB 
CHARACTER SET UTF8 
COLLATE UTF8_UNICODE_CI;
